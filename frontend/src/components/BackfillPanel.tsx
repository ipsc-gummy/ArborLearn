...
export function BackfillPanel({ node }: BackfillPanelProps) {
  ...
  const applyPatch = async () => {
    ...
    try {
      ...
      setError(applyError instanceof Error ? applyError.message : "应用回填失败");
    } finally {
      setIsApplying(false);
    }
  };
  ...
  return (
    <div className="relative">
      ...
      {open && (
        <div className="tl-panel absolute right-0 top-11 z-50 w-[min(32rem,calc(100vw-2rem))] rounded-2xl border p-4 text-sm shadow-panel">
          <div className="mb-3">
          ...
          <div className="mt-3 flex justify-end gap-2">
            ...
            <Button variant="primary" size="sm" onClick={applyPatch} disabled={isApplying || !replacementText.trim()}>
